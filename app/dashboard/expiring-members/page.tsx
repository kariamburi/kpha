...
export default async function ExpiringMembersPage({
    searchParams,
}: {
    ...
    const expiring7Days = members.filter((m) => daysUntil(m.expiryDate) <= 7).length;
    const expiring14Days = members.filter((m) => daysUntil(m.expiryDate) <= 14).length;
    const expiring30Days = members.length;
    ...
    return (
        <div className="space-y-5">
                ...
                <div className="mt-1 flex flex-col justify-between gap-4 md:flex-row md:items-center">
                    <div>
                        ...
                        <p className="mt-2 text-sm font-semibold text-slate-500">
                            Members whose subscriptions expire within the next 30 days.
                        </p>
                    </div>

                    <Link
                        href="/dashboard"
                        className="rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-black text-slate-800 transition hover:bg-slate-50"
                    >
                        ← Dashboard
                    </Link>
                </div>
            ...
            <form
                action="/dashboard/expiring-members"
            ...
            <div className="grid gap-4 md:grid-cols-3">
                <StatCard title="Expiring in 7 Days" value={expiring7Days.toString()} />
                <StatCard title="Expiring in 14 Days" value={expiring14Days.toString()} />
                <StatCard title="Expiring in 30 Days" value={expiring30Days.toString()} />
            </div>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        ...
                        <tbody>
                            {paginatedMembers.length === 0 ? (
                                <tr>
                                    ...
                                    >
                                        No members expiring within 30 days.
                                    </td>
                                </tr>
                            ) : (
                ...
                <div className="mt-4 flex flex-wrap items-center justify-end gap-3 border-t pt-4 text-sm">
                    ...
                    <Link
                        href={`/dashboard/expiring-members?${prevQuery.toString()}`}
                    ...
                    <Link
                        href={`/dashboard/expiring-members?${nextQuery.toString()}`}
